...
public class User {
    ...
    public static User getUser(String email) throws SQLException {
        List<Object> row = Sql.getInstance().getRow("SELECT user_id,password,account_type FROM user where email=?", email);
        User user = null;
        if (row != null) {
    ...
    public static User getUser(int userId) throws SQLException {
        List<Object> row = Sql.getInstance().getRow("SELECT email,password,account_type FROM user where user_id=?", userId);
        User user = null;
        if (row != null) {
    ...
    public static boolean isRegisteredEmail(String email) throws SQLException {
        return Sql.getInstance().execute("SELECT user_id FROM user WHERE email=?", email);
    }

    public static boolean authenticateUser(String email, String password) throws SQLException {
        return Sql.getInstance().execute("SELECT email FROM user WHERE email=? AND password=?", email, password);
    }

    public static User createUser(String email, String password, UserType userType) throws Exception {
        int userId =1;
        Sql.getInstance().execute("INSERT INTO user (email,password,account_type,date_created) VALUES(?,?,?,datetime('now'))", email, password, userType.getName());
